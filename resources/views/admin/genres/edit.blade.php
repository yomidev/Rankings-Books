...
<div class="max-w-3xl mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
    ...
    <form action="{{route('admin.genres.update', $genre->id)}}" method="POST">
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" cols="30" rows="10" class="w-full p-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500">{{$genre->description}}</textarea>
            @error('name')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
